...
function update!(opt::Adam, mse, X, y, n_steps) 
    for i in 1:n_steps
        ...
        opt.tmp_theta .-= ((opt.eta .* sqrt.(1.0 - opt.beta[2]^i)) ./ (sqrt.(1.0 .- opt.beta[1]^i))) .* opt.m ./ (sqrt.(opt.v) .+ opt.eps)  
    end
end
...
function update_circuit_param!(opt::Adam, cost, n_steps) 
    for i in 1:n_steps
        #g = get_cost_gradient(mse, X, y, opt.tmp_theta)
        g = generate_numerical_grad(cost)
        g = g(opt.tmp_theta)
...
function solve_gs_Adam(
    ...
    update_circuit_param!(adam, cost, n_steps) 
