...
function gen_t1_diag_kucj(a; sgn=1.0)
    #a^\dagger_a a_i (excitation)
    ...
    for i = 1:k
        ...
        for a_spatial in (1:norb)
            for ispin1 = 1:2
                ...
                generator = gen_t1_diag_kucj(a_spin_orbital, sgn=-1.0)
                add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                paraminfo_t1_diag = ((a_spin_orbital, i), num_theta(circuit))
        ...
        for (a_spatial, i_spatial) in (Iterators.product(1:norb, 1:norb))
            for ispin1 = 1:2
                ...
                #paraminfo_t1_nondiag_real_up = ((a_spatial, ispatial, real=1, k), num_theta(circuit)) 
                paraminfo_t1_nondiag_real = ((a_spatial, i_spatial, 1, i), num_theta(circuit) )
            ...
            for ispin1 = 1:2
                ...
                add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                # paraminfo_t1_nondiag_imag = ((a_spatial, i_spatial, imag=2, i), num_theta(circuit))
                paraminfo_t1_nondiag_imag = ((a_spatial, i_spatial, 2, i), num_theta(circuit))
        ...
        for a in 1:norb
            ...
            add_parametric_circuit_using_generator!(circuit, generator, 0.0)
            paraminfo_t2_diag = ((a, a, 1, 2, k), num_theta(circuit))
            ...
            for b in 1:norb
                ...
                for spin_a in 1:1
                    for spin_b in 1:1
                        ...
                        generator = rm_identity(generator)
                        add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                        paraminfo_t2_nondiag_spin_diag1 = ((a, b, 1, 1, k), num_theta(circuit))
                        push!(parameterinfo, paraminfo_t2_nondiag_spin_diag1)

                        # a, b , spin_a=2, spin_b=2
                        aa = so_idx(a, spin_a+1)
                        bb = so_idx(b, spin_b+1)
                        generator = gen_t2_kucj_2(aa, bb)
                        generator = rm_identity(generator)
                        add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                        paraminfo_t2_nondiag_spin_diag2 = ((a, b, 1, 1, k), num_theta(circuit))
                        push!(parameterinfo, paraminfo_t2_nondiag_spin_diag2)
                    end
                end
            end
        end
        # non-diagonal with respect to spin-orbitals
        for a in 1:norb
            for b in 1:norb
                # a<bのみループが回るようにしたい
                if a >= b
                ...
                for spin_a in 1:1
                    for spin_b in 2:2
                        ...
                        generator = rm_identity(generator)
                        add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                        paraminfo_t2_nondiag_spin_nondiag1 = ((a, b, spin_a, spin_b, k), num_theta(circuit))
                        push!(parameterinfo, paraminfo_t2_nondiag_spin_nondiag1)
                        ...
                        generator = rm_identity(generator)
                        add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                        paraminfo_t2_nondiag_spin_diag2 = ((a, b, spin_a, spin_b, k), num_theta(circuit)) #same parainfo as spin_a=1, spin_n=2
                        push!(parameterinfo, paraminfo_t2_nondiag_spin_diag2)
                    end
                end
            end
