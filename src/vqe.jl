...
#using SciPy: SciPy
export solve_gs, solve_gs_kucj_sampling
...
function _generate_cost_solve_gs_sampling(circuit, state0, ham_qubit, nshots, comm)

    # Define a cost function
    function cost(theta_list)
        #println("cost called = $(MPI.Comm_rank(comm))")
        update_circuit_param!(circuit, theta_list)
        state = copy(state0)
        update_quantum_state!(circuit, state)
        
        # 微分を計算するためにコスト関数を計算するとき→ allreduceしない
        #println("before sampling = $(MPI.Comm_rank(comm))")
        res::Float64 = get_expected_value_sampling(ham_qubit, state, nshots=nshots)
        #println("after sampling = $(MPI.Comm_rank(comm))")
        ...
        #println("before Allreduce = $(MPI.Comm_rank(comm))")
...
function solve_gs_sampling(
    ...
    # Define a cost function
    cost = _generate_cost_solve_gs_sampling(circuit, state0, ham_qubit, nshots, comm)
    #cost_nompi = _generate_cost_solve_gs_sampling(circuit, state0, ham_qubit, nshots, nothing)
    ...
    function callback(x)
        #@show "callback begin"
        #println("before callback = $(MPI.Comm_rank(comm))")
        push!(cost_history, cost(x))
        if verbose && rank == 0
            println("iter ", length(cost_history), " ", cost_history[end])
        end
        #@show "callback end"
        #println("after callback = $(MPI.Comm_rank(comm))")
    end
    # opt = SciPy.optimize.minimize(...)
...
function solve_gs_kucj_sampling(
    ham_qubit::QubitOperator,
    circuit::VariationalQuantumCircuit,
    state0::QuantumState,
    keys;
