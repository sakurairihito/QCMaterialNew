...
function kucj(n_qubit; conserv_Sz_singles=true, k=1, ucj=true)
    if n_qubit <= 0 || n_qubit % 2 != 0
        error("Invalid n_qubit: $(n_qubit)")
    end
    ...
    for (a_spatial, i_spatial) in (Iterators.product(norb, norb))
        for ispin1 = 1:2, ispin2 = 1:2
            if conserv_Sz_singles && sz[ispin1] + sz[ispin2] != 0
                continue
            end
            ...
            generator = gen_t1(a_spin_orbital, i_spin_orbital)
            add_parametric_circuit_using_generator!(circuit, generator, 0.0)
            orbital_rotation = ("orb_rot",  num_theta(circuit)) # ここは全て独立なパラメータ
    ...
    for i = 1:k
        ...
        for (a_spatial, i_spatial) in (Iterators.product(1:norb, 1:norb))
            for ispin1 = 1:2
                ...
                #paraminfo_t1_nondiag_real_up = ((a_spatial, ispatial, real, k), num_theta(circuit)) 
                # non-diag real
                paraminfo_t1_nondiag_real = (a_spatial, i_spatial, "real", i) 
            ...
            for ispin1 = 1:2
                ...
                add_parametric_circuit_using_generator!(circuit, generator, 0.0)
                # paraminfo_t1_nondiag_imag = ((a_spatial, i_spatial, imag, i), num_theta(circuit))
                paraminfo_t1_nondiag_imag = (a_spatial, i_spatial, "imag", i)
