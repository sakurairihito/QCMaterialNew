...
#=
ispython(circuit::ParametricQuantumCircuit) = isa(circuit, PyObject)

function set_parameter!(circuit::ParametricQuantumCircuit, index, theta)
    if index <= 0 || index > get_parameter_count(circuit)
        error("index is out of range!")
    end
    if ispython(circuit)
...
#function create_simul_gate(circuit::QulacsParametricQuantumCircuit,idx_qubit::Int, thetas)
#  theta1 = thetas[1]
#  theta2 = thetas[2]
#  #CNOT02 = CNOT(first, second)
#  add_parametric_RZ_gate!(circuit,idx_qubit, -theta[2])
#  add_parametric_RY_gate!(circuit,idx_qubit, theta[1])
#  add_parametric_RZ_gate!(circuit,idx_qubit, theta[2])
#  add_Z_gate!(circuit, idx_qubit)
